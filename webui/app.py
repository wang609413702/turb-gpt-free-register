...
def _account_secret_value(row: dict, field: str) -> str:
    ...
    raise ValueError("field 仅支持 access_token/copy_line/codex_agent_token/original_email_line/totp_secret/totp_code/password")
...
def create_app(auth_code: str | None = None) -> Flask:
    ...
    @app.post("/api/accounts/momo-check")
    def api_account_momo_check():
        """把单账号 MoMo 检测加入后台队列。Body {account_id|email, proxy?}"""
        data = request.get_json(silent=True) or {}
        acc_id = data.get("account_id") or data.get("id")
        email = (data.get("email") or "").strip()
        acc = None
        ...
        if not token:
            return jsonify({"ok": False, "error": "该账号没有 access_token"}), 400
        account_id = int(acc.get("id"))
        queued = momo_check_service.enqueue_account_momo_check(
            account_id=account_id,
            email=acc.get("email") or "",
            access_token=token,
            trigger="manual",
    ...
    @app.post("/api/accounts/momo-check-bulk")
    def api_accounts_momo_check_bulk():
        """批量把 MoMo 检测加入统一后台队列。Body {account_ids:[...], proxy?}"""
        data = request.get_json(silent=True) or {}
        ids = data.get("account_ids") or data.get("ids") or []
        if not isinstance(ids, list) or not ids:
            return jsonify({"ok": False, "error": "account_ids 必须是非空数组"}), 400
