...
# ============================== MoMo 检测 ============================== #
def claim_account_momo_check(
    acc_id: int | None = None,
    email: str | None = None,
    trigger: str = "manual",
) -> bool:
    """原子占用账号的 MoMo 检测；已有未超时检测时返回 False。"""
    with _LOCK:
        accounts = _load_accounts()
        target_email = (email or "").lower()
        row = next((
        ...
        current_status = row.get("momo_check_status")
        if current_status in {"queued", "running"}:
            try:
                stamp_key = "momo_check_queued_at" if current_status == "queued" else "momo_check_started_at"
                stale_after = _PLAN_CHECK_QUEUE_STALE_SECONDS if current_status == "queued" else _PLAN_CHECK_STALE_SECONDS
                started_at = datetime.fromisoformat(str(row.get(stamp_key) or ""))
                if (datetime.now() - started_at).total_seconds() < stale_after:
                    return False
        ...
        now = _now()
        row["momo_check_status"] = "queued"
        row["momo_check_trigger"] = str(trigger or "manual")
        row["momo_check_queued_at"] = now
        row["momo_check_started_at"] = None
...
def mark_account_momo_check_running(acc_id: int) -> bool:
    """把已排队的 MoMo 检测标记为执行中。"""
    with _LOCK:
        accounts = _load_accounts()
        row = next((r for r in accounts if int(r.get("id") or 0) == int(acc_id)), None)
        if row is None or row.get("momo_check_status") not in {"queued", "running"}:
            return False
        row["momo_check_status"] = "running"
...
def update_account_momo_check(acc_id: int | None = None, email: str | None = None, result: dict | None = None) -> bool:
    """更新账号 MoMo 支付检测结果。"""
    result = result or {}
    with _LOCK:
        accounts = _load_accounts()
        ...
        ok = bool(result.get("ok"))
        row["momo_check_status"] = "success" if ok else "failed"
        row["momo_ok"] = ok
        row["momo_checked_at"] = result.get("checked_at") or _now()
        row["momo_check_completed_at"] = _now()
        row["momo_check_http_status"] = result.get("http_status")
        row["momo_check_error"] = None if ok else result.get("error")
...
def recover_interrupted_momo_checks() -> int:
    """服务启动时把上次进程遗留的 MoMo 检测状态恢复为可重试失败。"""
    with _LOCK:
        accounts = _load_accounts()
        recovered = 0
        now = _now()
        for row in accounts:
            if row.get("momo_check_status") not in {"queued", "running"}:
                continue
            row["momo_check_status"] = "failed"
            row["momo_ok"] = False
            row["momo_check_error"] = "WebUI 重启导致 MoMo 检测中断，请重新检测"
            row["momo_check_completed_at"] = now
            row["updated_at"] = now
            recovered += 1
        if recovered:
            _save_accounts(accounts, sync_artifacts=False)
...
def _load_rebind_emails() -> list[dict]:
    rows = _read_json(_REBIND_EMAIL_JSON, [])
    return rows if isinstance(rows, list) else []
...
def _save_rebind_emails(rows: list[dict]) -> None:
    ...
    _write_json(_REBIND_EMAIL_JSON, rows)
    available_rows = [r for r in rows if r.get("status") == "available"]
    lines = [_rebind_email_line(r) for r in sorted(available_rows, key=lambda x: int(x.get("id") or 0))]
    _REBIND_EMAIL_TXT.write_text(("\n".join(lines) + ("\n" if lines else "")), encoding="utf-8")
