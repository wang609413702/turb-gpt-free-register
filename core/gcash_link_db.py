...
提链记录存在 JSON 文件 gcash_link_records.json；代理池缓存到 gcash_link_proxy_cache.json（重启后保留）。
不依赖账号表，和账号页面完全分离。
"""
...
_RECORDS_FILE = Path(__file__).resolve().parent.parent / "gcash_link_records.json"
_PROXY_CACHE_FILE = Path(__file__).resolve().parent.parent / "gcash_link_proxy_cache.json"
_PROMO_PROXY_CACHE_FILE = Path(__file__).resolve().parent.parent / "gcash_link_promo_proxy_cache.json"
    ...
    with _LOCK:
        ...
        for acc in accounts:
            ...
            if not email or email.lower() in existing_emails:
                continue
            record = {
                "id": f"gl_{int(time.time() * 1000)}_{len(records)}",
