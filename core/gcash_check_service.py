...
镜像 core/momo_check_service.py，使用独立 GCash 代理池。
"""
...
from config.proxy import pick_gcash_proxy
from core import db
from core.chatgpt_momo import check_account_gcash
...
_WORKERS = _int_setting("GCASH_CHECK_WORKERS", 3, 1, 16)
_QUEUE_LIMIT = _int_setting("GCASH_CHECK_QUEUE_LIMIT", 500, _WORKERS, 5000)
...
def _wait_for_rate_slot() -> None:
    ...
    min_interval = _float_setting("GCASH_CHECK_MIN_INTERVAL", 0.4, 0.0, 30.0)
    jitter = _float_setting("GCASH_CHECK_JITTER", 0.3, 0.0, 30.0)
    with _RATE_LOCK:
        now = time.monotonic()
...
def _run_account_gcash_check(
    *,
    ...
    try:
        ...
        # 前端未显式覆盖时，GCash 检测单独使用专用代理池轮换；池为空则直连。
        if proxy is None:
        ...
        if result.get("ok"):
            ...
            logger.info(
                "[GCash] 后台检测成功: %s, decision=%s, has_gcash=%s, trigger=%s",
                email,
                result.get("decision"),
                has_target,
                trigger,
            ...
            logger.warning(
                "[GCash] 后台检测失败: %s, trigger=%s, error=%s",
                email,
                trigger,
        ...
        try:
            ...
            logger.exception("[GCash] 写入后台检测异常状态失败: account_id=%s", account_id)
...
def enqueue_account_gcash_check(
    *,
    ...
    if not _QUEUE_SLOTS.acquire(blocking=False):
        return {"accepted": False, "busy": False, "queue_full": True, "error": "GCash 检测队列已满，请稍后重试"}
    ...
    try:
        ...
        result = {
            "ok": False,
            "checked_at": datetime.now().isoformat(timespec="seconds"),
            "error": f"GCash 检测入队失败: {type(exc).__name__}: {str(exc)[:160]}",
            "decision": "checkout_failed",
        }
        db.update_account_gcash_check(acc_id=account_id, result=result)
