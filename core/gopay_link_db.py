...
提链记录存在 JSON 文件 gopay_link_records.json；代理池缓存到 gopay_link_proxy_cache.json（重启后保留）。
不依赖账号表，和账号页面完全分离。
"""
...
_RECORDS_FILE = Path(__file__).resolve().parent.parent / "gopay_link_records.json"
_PROXY_CACHE_FILE = Path(__file__).resolve().parent.parent / "gopay_link_proxy_cache.json"
_PROMO_PROXY_CACHE_FILE = Path(__file__).resolve().parent.parent / "gopay_link_promo_proxy_cache.json"
    ...
    with _LOCK:
        ...
        for acc in accounts:
            ...
            if not email or email.lower() in existing_emails:
                continue
            record = {
                "id": f"gpl_{int(time.time() * 1000)}_{len(records)}",
...
# ==================== promo 代理池（第二个池，checkout/update 用） ==================== #
def _load_promo_proxy_cache() -> list[str]:
    if not _PROMO_PROXY_CACHE_FILE.exists():
        return []
    try:
        data = json.loads(_PROMO_PROXY_CACHE_FILE.read_text(encoding="utf-8"))
        return [str(p) for p in data] if isinstance(data, list) else []
    except Exception:
        return []


def _save_promo_proxy_cache(proxies: list[str]) -> None:
    _PROMO_PROXY_CACHE_FILE.write_text(json.dumps(proxies, ensure_ascii=False, indent=2), encoding="utf-8")


def get_promo_proxy_pool() -> list[str]:
    with _LOCK:
        return _load_promo_proxy_cache()


def set_promo_proxy_pool(text: str) -> list[str]:
    """从多行文本解析 promo 代理池并存入缓存文件。每行一条，自动归一化。"""
    from config.proxy import normalize_proxy
    raw_lines = [line.strip() for line in str(text or "").splitlines() if line.strip()]
    normalized = [normalize_proxy(line) for line in raw_lines]
    valid = [p for p in normalized if p]
    with _LOCK:
        _save_promo_proxy_cache(valid)
    return valid


def pick_promo_proxy() -> str:
    """从 promo 代理池随机取一个；空返回 ""（直连）。"""
    import random
    with _LOCK:
        pool = _load_promo_proxy_cache()
    if not pool:
        return ""
    return random.choice(pool)
